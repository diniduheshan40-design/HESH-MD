...
> ⚡ ᴘᴏᴡᴇʀᴇᴅ ʙʏ ʜᴇꜱʜᴀɴ-ᴍᴅ ⚡`.trim();
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
      ...
      for (const msg of messages) {
        ...
        // 🟢 2. SENDER RESOLVER & OWNER DETECTION
        ...
        // Make sure it's NOT a self-message generated by the bot account
        const isFromBot = msg.key.fromMe || (myBotNum && cleanSenderNum === myBotNum);

        // Check if message is from Owner numbers
        const isOwner = !isFromBot && global.OWNER_NUMBERS.some(owner => 
          cleanSenderNum.includes(owner) || 
          cleanContextNum.includes(owner)
        );
        ...
        // 🟢 3. STRICT OWNER REACT (94719845166 සඳහා පමණි - Baileys Reaction Format)
        if (global.botSettings?.ownerReact && isMasterCreator && !isFromBot) {
          try {
            ...
            console.error('Owner React Error:', reactErr.message);
          }
        }

        // 🟢 4. AUTHORIZED CONTROLLER (Owner or Host)
        const isAuthorizedToControl = isOwner || msg.key.fromMe;
        ...
        if (!isFromBot && !isAuthorizedToControl && !isGroup && global.botSettings?.autoAiInbox) {
          try {
